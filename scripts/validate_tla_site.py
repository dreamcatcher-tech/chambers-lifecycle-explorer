...
def validate_projection(payload: dict) -> None:
    ...
    if authority.get("release") != "chambers-formal-specification/v1.1.0":
        fail("formal release identity drifted")
    if authority.get("version") != "1.1.0":
        fail("formal release version drifted")
    if authority.get("gitTag") != "formal-spec-v1.1.0":
        fail("formal release tag drifted")
    if authority.get("releaseKind") != "semantic_baseline":
        fail("formal release kind drifted")
    if authority.get("semanticDelta") != "adds ordinary snapshot-read authority, ephemeral Chamber identity/admission and minimal Vault seam, ordinary generation upgrades, bounded interface composition, and a baseline-complete jurisdiction finding":
        fail("formal release semantic delta drifted")
    if authority.get("supersedes") != "chambers-formal-specification/v1.0.1":
        fail("formal release predecessor drifted")
    if authority.get("manifestSha256") != "a4e5431c867751692dbe4a5f7395c6b362212b6a6a62bc2bd41e6fdd31fc649f":
    ...
    if len(coverage.get("releaseKernels", [])) != 11:
        fail("formal release kernel inventory is incomplete")

    models = payload.get("models", [])
    if [model.get("id") for model in models] != [
        "ark_core_appliance", "multi_ark", "host_cutover", "baseline_composition"
    ]:
    ...
    if len(controls) != 12 or any(control.get("status") != "expected_counterexample" for control in controls):
