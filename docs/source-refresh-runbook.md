...
6. Run the complete local gate:
   ...
   Browser checks must exercise every registered workspace and sequence. For call selection, assert that the requested call becomes current, `scrollLeft` remains unchanged, an already visible call does not move the page, and an off-screen call uses the primary page scrollbar for only the minimum top/bottom reveal required. The diagram must not own a vertical scrollbar. While a long diagram crosses the page viewport, assert that its sticky actor labels remain below sticky page controls, mirror every actor, stay horizontally aligned after panning, and are included in selected-call occlusion geometry. Also sweep representative calls to prove the active sequence's inspector height is stable and unclipped. Exercise reset, sidebar/mobile/search selection, scrubber, next/previous, playback restart, Map and Functions drill-down, document switching, URL history with Back/Forward restoration, keyboard focus, and accessibility roles.
