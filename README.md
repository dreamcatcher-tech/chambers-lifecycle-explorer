...
- **Legible Chambers startup ladder** — Chambers opens with **First Ark Core installation**, **Selected Ark Core cold start**, **Ark Core bootstrap**, **Whole-appliance crash recovery**, **Scope-bound child Ark Core activation**, then **Ordinary activation**. One atomic `boot-control/selected.json` names one OCI image and one gVisor Core task. s6 is PID 1, seeds private `/run/iii` tmpfs, and starts Engine, Persistence, Gateway, and Supervisor in order; any required process or semantic-readiness loss exits the whole task rather than restarting one member. Exact workers use loopback `127.0.0.1:49133`; ProcMan and ordinary descendants use Gateway-authorized Ark-private port `49134`, never a published host port. ProcMan reads the selector once, attaches one Core-only volume, creates per-task private-network attachments, and denies forwarding between scope networks without learning the internal process graph. Whole-task loss replaces the complete scope tree. A child Core rejects caller-supplied scope routing, receives its own selector, test volume, private network, and descendant-only ProcMan authority. The source records a 22/22 bounded runsc mechanism proof while keeping s6 fatality and production containerd/CNI/storage integration explicitly pending. Gateway warm cutover remains ordinary-only, and one pre-authorized last-known-good selector may be consumed before ordinary admission/effects.
