...
- **Legible Chambers startup ladder** — Chambers opens with **First boot installation**, **Selected Boot set cold start**, **Boot control bootstrap**, **Reboot selected Boot set**, then **Ordinary activation**. One protected containerd image record, `dreamcatcher/bootset:current`, selects an immutable root binding exact Bootstrap Engine, Router, Persistence, and Supervisor Covenant Realizations. The Host Agent is the sole containerd client and starts or reuses those four separate Chambers in order. Router alone receives the protected control-listener capability; Persistence recovers durable desired routing and Supervisor drives live-route reconciliation. Router and Supervisor replace one another sequentially while same-Engine cutover changes one control member at a time. Ordinary lifecycle uses the typed `chamber::activate`, `chamber::inspect`, and `chamber::stop` I3 surface. Selected/predecessor Boot-set member closures are product-durable while the ordinary runtime namespace is reconstructable. An accepted Builder image may be imported at genesis but runs only as a separate ordinary Covenant, so build never enters Host Agent or cold start.
...
`install_boot_seed`, `wake_bootset`, `deliver_final_reply`, and `containerd_*` as conventional host-boundary
calls. Typed `chamber::*` and `bootset::*` functions remain I3 calls registered by the Host Agent after Router
readiness, while `routing::*` functions are owned by the separate Router Covenant. The current Cardflow
