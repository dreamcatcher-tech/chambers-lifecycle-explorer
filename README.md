...
- **Legible Chambers startup ladder** — Chambers opens with **First core installation**, **Engine cold start**, **Core bootstrap**, **Reboot selected Core**, then **Ordinary activation**. Persistence owns exact current selections and atomically publishes `core-current.json` in the same durable backup domain; Procman reads and verifies those bytes but never owns a shadow selection ledger. Missing normal boot state fails closed—only an externally accepted one-use Boot Seed may establish genesis. The host-resident minimum Image Materializer imports or retrieves exact authorized content for `containerd`; richer build/provider logic can run in upgradeable Chambers. `containerd` remains a disposable cache and never selects or calls Persistence or I3 directly.
