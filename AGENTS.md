...
- For Chambers, preserve `procman` as the leftmost lane whenever present. If host materialization is shown, Image Materializer and `containerd` follow it before the trusted host runtime; otherwise the runtime may immediately follow `procman`. Every `activate_chamber`/`stop_chamber` call targets that runtime while a source note identifies the exact Chamber subject. Keep Persistence and `containerd` separate: `procman` reads durable launch data, Image Materializer alone calls `containerd`, and the containerd storage slice remains disposable.
