...
- For Chambers, preserve `procman` as the leftmost lane whenever present. A physical-effect sequence places the trusted host runtime immediately after it, and every `activate_chamber`/`stop_chamber` call targets that runtime while a source note identifies the exact Chamber subject.
